...
const PredictiveConsumption = () => {
  ...
  const availableAlgorithms = useMemo(() => {
    if (!warehouseData.length) return algorithmOrder;
    const available = Array.from(new Set(warehouseData.map((item) => item.algorithm))) as AlgorithmType[];
    return algorithmOrder.filter((algorithm) => available.includes(algorithm));
  }, [warehouseData]);
  ...
  }, [availableAlgorithms, warehouseData.length]);
